...
PP(pp_ucfirst)
{
	...
	doing_utf8 = TRUE;
	    ...
	    _toLOWER_utf8_flags(s, s + slen, tmpbuf, &tculen, IN_LC_RUNTIME(LC_CTYPE));
             ...
             * says we are supposed to remove the COMBINING DOT ABOVE.  Test
             * for this situation. */
...
#define COMBINING_GREEK_YPOGEGRAMMENI 0x0345
	...
	while (s < send) {
	    ...
	    if (     UNLIKELY(in_iota_subscript)
                && ! is_COMBINING_MARK_utf8_safe(s, send))
            {
   ...
   || (UNICODE_MAJOR_VERSION == 2 && UNICODE_DOT_VERSION >= 1		\
			...
			*d++ = 'S'; *d = 'S'; /* upper case is 'SS' */
		    ...
		    d = (U8*)SvPVX(dest) + len;
                     ...
                     * converting to upper and UTF-8.  If a resulting byte is
                     * invariant in UTF-8, output it as-is, otherwise convert
                     * to UTF-8 and append it to the output.
                     *
...
PP(pp_lc)
{
    ...
    if (IN_LC_RUNTIME(LC_CTYPE)) {
            ...
            &&   (next_I = (U8 *) memchr(s, 'I', len)))
        {
            ...
            sv_utf8_upgrade_flags_grow(dest, 0, send - s
                                              + I_count
