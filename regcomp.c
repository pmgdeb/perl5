...
#define STRLENs(s)  (sizeof("" s "") - 1)
    ...
    if (equals_pos >= 0) {
        ...
        if (name[i] == '[') {
            ...
            if (table_index) {
                ...
                SV * pattern = Perl_newSVpvf(aTHX_ "(?iaa:\\A%.*s\\z)", (unsigned) name_len - i, name + i);

                REGEXP * subpattern = re_compile(pattern, 0);
                while (*prop_values) {
                    const char * const entry = *prop_values;
                    const Size_t len = strlen(entry);
                    SV* entry_sv = newSVpvn(entry, len);

                    if (pregexec(subpattern, (char *) entry, (char *) entry + len, (char *) entry, 0, entry_sv, 0)) {
    ...
    if (could_be_user_defined) {
        ...
        if (user_sub) {
            ...
            PERL_INT_FAST8_T retry_countdown = 30;
